    ...
    <body>  
      <div id="content" class="white">      
       <div>
		...
		<c:choose>
			<c:when test="${not empty chartsList }">
				<c:forEach items="${chartsList }" var="charts">
					<tr class="main_info">
						<td>${charts.chartsId }</td>
						<td>${charts.charts1 }</td>
						<td>${charts.charts2 }</td>
						<td>${charts.charts3 }</td>
					</tr>
				</c:forEach>
			</c:when>
			<c:otherwise>
